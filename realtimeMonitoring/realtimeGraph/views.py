...
def get_temperature(request, **kwargs):
    data_result = {}
    ...
    try:
        stationParam = request.GET.get("station", None)
    except:
        tempParam = 30

    station = Station.objects.filter(id=stationParam)[0]
    ...
    try:
        tempParam = request.GET.get("value", None)
    except:
        tempParam = 30
    ...
    temperatureStationData = Data.objects.filter(station_id=station.id,
                                                 measurement__name='Temperatura',
                                                 time__gte=start_ts,
                                                 time__lte=end_ts)
    for tempData in temperatureStationData:
        #print(tempData)
        filtered_values = list(filter(lambda x: x > float(tempParam), tempData.values))
        timeStr = str(tempData.time)
        data.append(
            {
                "name": f"{station.location.city.name}, {station.location.state.name}, {station.location.country.name}",
                "values": filtered_values,
                "count": len(filtered_values),
                "time": tempData.base_time
    ...
    data_result["station"] = station.id
    data_result["start"] = startFormatted
    data_result["end"] = endFormatted
    data_result["data"] = data
